...
public class BoidController_old : MonoBehaviour
{
    [SerializeField] ColliderActivityTracker _radar;

    ColliderActivityInfo<BoidController_old> _radarData = new ColliderActivityInfo<BoidController_old>(c=>(c as Collider2D)?.attachedRigidbody?.GetComponent<BoidController_old>());
